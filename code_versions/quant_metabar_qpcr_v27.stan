 ...
 *
 * VERSION 27: FACTOR-ANALYTIC HSGP (K_f SHARED LATENT FIELDS)
 *
 * Changes from v22:
 *
 * 1. Replace N_species independent per-species HSGP fields with K_f shared
 *    latent fields (K_f << N_species).  Each species' spatial effect is a
 *    weighted sum of K_f latent fields via a lower-triangular loading matrix:
 *      spatial_effect[loc, sp] = sum_{k=1}^{K_f} loading[sp, k] * latent_field[loc, k]
 *
 * 2. Lower-triangular loading matrix resolves K_f*(K_f-1)/2 rotational and
 *    K_f sign-flip non-identifiabilities (requires N_species >= K_f).
 *    Positive diagonal elements (loading_diag[k]) fix orientation per factor.
 *
 * 3. gp_alpha_factor[K_f] is fixed data (default rep(1.0, K_f)), removing the
 *    multiplicative ridge between log_alpha_k, log_rho_k, and loading_diag.
 *    rho_factor[k] is the sole spatial scale per factor; loading[sp,k] is the
 *    sole amplitude for factor k.
 *
 * 4. Parameter count: K_f*M z_basis + K_f rho_factor + K_f loading_diag +
 *    (K_f*N_species - K_f*(K_f+1)/2) loading_offdiag.
 *    At N_species=6, K_f=3, M=20: 60 + 3 + 3 + 12 = 78 spatial params
 *    vs v22's 120 + 6 + 6 = 132.
 *
 * 5. No depth strata (contrast with v24/v25).
 *
...
data {
  ...
  // Factor-analytic HSGP spatial parameters (v27)
  int<lower=1> K_f;                            // number of shared latent fields
  real<lower=0> rho_min;                       // hard lower bound on rho_factor (km)
  real<lower=0> rho_factor_prior_mean;
  real<lower=0> rho_factor_prior_sd;
  vector<lower=0>[K_f] gp_alpha_factor;        // fixed GP amplitude per factor (default 1.0)
  real<lower=0> loading_prior_sd;              // regularizing prior on loading elements
  ...
  // Nugget SD priors (hierarchical across species, independent of loading amplitude)
...
transformed data {
  ...
  int N_loading_offdiag = K_f * N_species - K_f * (K_f + 1) / 2;
  if (N_species < K_f)
    reject("N_species (", N_species, ") must be >= K_f (", K_f,
           ") for lower-triangular loading. Reduce K_f.");
...
parameters {
  ...
  // K_f shared latent spatial fields (gp_alpha_factor fixed in data)
  vector<lower=rho_min>[K_f] rho_factor;
  matrix[M, K_f]              z_basis_factor;

  // Loading matrix: lower-triangular with positive diagonal.
  // Resolves K_f*(K_f-1)/2 rotational + K_f sign-flip non-identifiabilities.
  // Requires N_species >= K_f (checked in transformed data).
  vector<lower=0>[K_f]      loading_diag;
  vector[N_loading_offdiag] loading_offdiag_raw;

  // Nugget SD: hierarchical log-normal, INDEPENDENT of loading amplitude (from v21)
...
transformed parameters {
  ...
  // Nugget SD: own hierarchy, decoupled from loading amplitude (from v21)
  ...
  // K_f shared latent fields [N_locations, K_f], each mean-centered
  matrix[N_locations, K_f] latent_field;
  for (k in 1:K_f) {
    vector[M] spd_k = hsgp_2d_iso_spectral_weights(
        m_x, m_y, L_x, L_y, gp_alpha_factor[k], rho_factor[k]);
    vector[N_locations] raw_k = PHI * (spd_k .* z_basis_factor[, k]);
    latent_field[, k] = raw_k - mean(raw_k);
  }

  // Reconstruct lower-triangular loading matrix [N_species, K_f]
  matrix[N_species, K_f] loading;
  {
    int offdiag_idx = 1;
    for (j in 1:K_f) {
      for (i in 1:N_species) {
        if (i < j)
          ...
          loading[i, j] = loading_diag[j];
        else {
          loading[i, j] = loading_offdiag_raw[offdiag_idx];
          offdiag_idx  += 1;
        }
      }
    }
  ...
  // Species-specific spatial effects [N_locations, N_species]
  matrix[N_locations, N_species] spatial_effect;
  for (sp in 1:N_species)
    spatial_effect[, sp] = latent_field * to_vector(loading[sp, ]);
...
model {
  ...
  // Factor GP priors (gp_alpha_factor fixed in data; no prior needed)
  rho_factor                ~ normal(rho_factor_prior_mean, rho_factor_prior_sd);
  to_vector(z_basis_factor) ~ std_normal();

  loading_diag        ~ normal(0, loading_prior_sd);  // half-normal (lower=0)
  loading_offdiag_raw ~ normal(0, loading_prior_sd);

  // Nugget SD hierarchy — decoupled from loading amplitude (from v21)
...
generated quantities {
  ...
  // Cross-factor mean of length scale (v27)
  real rho_mean = mean(rho_factor);

#!/usr/bin/env Rscript
# sim_example_v27_full.R
#
# Full simulation: 100 locations, 6 species, K_f = 3 shared latent fields,
# m_x = 4, m_y = 5 (isotropic HSGP, 20 basis functions).
# 3 chains x 2000 iter (1000 warmup).
# Sources run_qpcr_metabar_workflow_v27.R for all utilities.
#
# Usage (run from project root):
#   Rscript code/spatial_qPCR_mb/sim_example_v27_full.R [options]
#
...
#   5  output_dir  (string,  default "/mmfs1/gscratch/coenv/rpkelly/output/sim_example_v27_full")
...
output_dir <- if (length(args) >= 5) args[5] else
  "/mmfs1/gscratch/coenv/rpkelly/output/sim_example_v27_full"

cat("=== sim_example_v27_full.R ===\n")
...
result <- run_workflow_v27(
  ...
  rho_factor_km       = 50,
  gp_alpha_factor_true = NULL,   # defaults to rep(1.0, K_f)
  loading_true        = NULL,    # random lower-triangular
...
# if (result$fit_successful) {
  ...
  if (!is.null(fit_obj) && inherits(fit_obj, "stanfit")) {
    ...
    rds_path <- file.path(output_dir, sprintf("fit_v27_%s.rds", ts))
    posterior_draws <- rstan::extract(fit_obj, permuted = FALSE)
    saveRDS(posterior_draws, rds_path)
    cat(sprintf("Stan fit object saved to: %s\n", rds_path))
  } else {
    cat(sprintf("WARNING: result[[\"fit\"]] is %s — skipping RDS save\n",
                class(fit_obj)))
  }

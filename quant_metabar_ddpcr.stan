// Quant MB model with ddPCR likelihood on the reference species.
//
// Extends quant_metabar_only.stan by adding a binomial-on-droplets likelihood:
//   positives[rep] ~ Binomial(total_droplets[rep], 1 - exp(-lambda_ref * v_drop))
// where lambda_ref = exp(log_concentration[loc, reference_species]) [copies/uL]
// and v_drop is the fixed droplet volume in uL (typically 0.000795 for QX200).
//
// No detection hurdle (ddPCR has no LOD-driven censoring) and no Normal-on-log-conc
// observation error — all uncertainty propagates from droplet count Poisson statistics.
...
data {
  ...
  // ===========================================
  // REFERENCE SPECIES (the species ddPCR measures)
  // ===========================================
...
transformed data {
  ...
  array[N_locations] int n_ddpcr_per_loc;
  ...
  for (rep in 1:N_ddpcr_reps) {
    n_ddpcr_per_loc[ddpcr_rep_location[rep]] += 1;
  }
}
...
model {
  ...
  // ============================================
  // ddPCR LIKELIHOOD — Binomial on positive droplets
  // Each well: positives ~ Binomial(N_drops, 1 - exp(-lambda_ref * v_drop))
  // ============================================
  for (rep in 1:N_ddpcr_reps) {
...
generated quantities {
  ...
  // ============================================
  // LOCATION-LEVEL EXPECTED REFERENCE LOG-CONC
  // ============================================
  vector[N_locations] expected_log_conc_ref;
  for (loc in 1:N_locations)
    expected_log_conc_ref[loc] = log_concentration[loc, reference_species];
  ...
  int n_ddpcr_locations = 0;
  for (loc in 1:N_locations) {
    if (has_edna[loc]  == 1) n_edna_locations  += 1;
    if (has_ddpcr[loc] == 1) n_ddpcr_locations += 1;
  }
  ...
  // ============================================
  // POSTERIOR PREDICTIVE — ddPCR (binomial draws)
  // ============================================
  ...
  // ============================================
  // LOG-LIKELIHOOD — ddPCR (per replicate)
  // ============================================
  ...
  vector[N_locations] log_lik_ddpcr_loc;
  for (loc in 1:N_locations) {
    log_lik_edna_loc[loc]  = 0;
    log_lik_ddpcr_loc[loc] = 0;
  }
  for (obs in 1:N_edna_obs)
    log_lik_edna_loc[edna_obs_location[obs]] += log_lik_edna[obs];
  for (rep in 1:N_ddpcr_reps)
    log_lik_ddpcr_loc[ddpcr_rep_location[rep]] += log_lik_ddpcr[rep];
}
